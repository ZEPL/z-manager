...
//Same as NewClientFromEnv() but overrides host and port
func NewClientFromEnvWithHost(host, port string) (*Client, error) {
	client, err := NewVersionedClientFromEnvWithHost(host, port, "")
	if err != nil {
		return nil, err
	}
	client.SkipServerVersionCheck = true
...
func NewVersionedClientFromEnvWithHost(host, port, apiVerStr string) (*Client, error) {
	dockerEnv, err := getDockerEnv()
	if err != nil {
		return nil, err
	}
	dockerHost := "http://" + host + ":" + port
	if dockerEnv.dockerTLSVerify {
		...
		return NewVersionedTLSClient(dockerHost, cert, key, ca, apiVerStr)
	}
	return NewVersionedClient(dockerHost, apiVerStr)
}
