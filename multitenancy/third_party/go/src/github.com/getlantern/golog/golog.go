...
func (l *logger) Debug(arg interface{}) {
	_, err := fmt.Fprintf(getOutputs().debugOut, l.timePrefix()+"%s\n", arg)
	if err != nil {
		errorOnLogging(err)
	}
}
...
func (l *logger) Debugf(message string, args ...interface{}) {
	_, err := fmt.Fprintf(getOutputs().debugOut, l.timePrefix()+message+"\n", args...)
	if err != nil {
		errorOnLogging(err)
	}
}
...
func (l *logger) Error(arg interface{}) {
	_, err := fmt.Fprintf(getOutputs().errorOut, l.timePrefix()+"%s\n", arg)
	if err != nil {
		errorOnLogging(err)
	}
}
...
func (l *logger) Errorf(message string, args ...interface{}) {
	_, err := fmt.Fprintf(getOutputs().errorOut, l.timePrefix()+message+"\n", args...)
	if err != nil {
		errorOnLogging(err)
	}
}
