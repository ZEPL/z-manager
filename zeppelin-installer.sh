...
get_installation_type() {
  ...
  echo '1. Default (Hadoop 2.4.0; Spark 1.3.1)'
  echo '2. Advanced (choose your own versions)'
...
install_confirmation() {
  ...
  print_if_not_empty  "Spark master" "$SPARK_MASTER_URL"
...
show_history () {
  ...
  print_if_not_empty  "Spark master" "${user_params[2]}"
...
get_new_settings() {
  ...
  if [[ "${yn}" =~ $re_y ]]; then
    get_cluster_manager
    resource_manager="$?"
    get_master_url "${resource_manager}"
    get_spark_home "${resource_manager}"
    get_extra_cluster_params "${resource_manager}"
  fi
